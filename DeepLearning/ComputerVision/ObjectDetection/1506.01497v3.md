...
* 采用NMS非极大值抑制（详见XXX）等过滤手段，保留部分RPN生成的预测框
* **输出**：预测框生成的proposal
...
$$ Loss({p_i}, {t_i}) = \frac{1}{N_{cls}}\sum_iL_{cls}(p_i, p_i^*)+\lambda \frac{1}{N_{reg}}\sum_ip_i^*L_{reg}(t_i, t_i^*) $$

$$ L_{reg}(t_i, t_i^*)=\sum_{j\in{x, y, w, h}}{\rm smooth}_{L_1}(t_{ji}-t_{ji}^*) $$
...
$t_i$代表坐标向量，$t_i^*$是真实框的坐标向量。$x, y, w, h$分别代表边框中心的坐标和宽度长度。$x, x_a, x^*$分别代表预测框，anchor，真实框。
